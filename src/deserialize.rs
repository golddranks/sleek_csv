...
pub fn deserialize_byte_record_arena<'de, D: Deserialize<'de>>(
    ...
    for (i, record) in records.iter().enumerate() {
        let mut deser = DeRecordWrap(DeByteRecord {
            it: record.iter().peekable(),
            headers: None, // headers.map(|r| r.iter()),
            field: 0,
        });
...
"01000002","R2000119","0040001","&1","04101","20170923","台湾・古龍華","ｺﾘﾕｳｶ","19981221",2019/10/09 8:10:09,"C006"
"#;
...
mod tests {
    ...
    fn de_headers<D: DeserializeOwned>(headers: &[&str], fields: &[&str]) -> Result<D, Error> {
    ...
    fn with_header() {
        ...
        let got: Foo = de_headers(&["x", "y", "z"], &["hi", "42", "1.3"]).unwrap();
        assert_eq!(
            got,
            Foo {
                x: "hi".into(),
    ...
    #[test]
    fn map_headers() {
        let got: HashMap<String, i32> = de_headers(&["a", "b", "c"], &["1", "5", "10"]).unwrap();
        assert_eq!(got.len(), 3);
        assert_eq!(got["a"], 1);
        assert_eq!(got["b"], 5);
        assert_eq!(got["c"], 10);
    ...
    fn enum_untagged() {
        ...
        let got: Row = de_headers(&["x", "y", "z"], &["true", "null", "1"]).unwrap();
        assert_eq!(
            got,
            Row {
                x: Boolish::Bool(true),
    ...
    fn option_empty_field() {
        ...
        let got: Foo = de_headers(&["a", "b", "c"], &["", "foo", "5"]).unwrap();
        assert_eq!(
            got,
            Foo {
                a: None,
    ...
    fn option_invalid_field() {
        ...
        let got: Foo = de_headers(&["a", "b", "c"], &["xyz", "", "5"]).unwrap();
        assert_eq!(
            got,
            Foo {
                a: None,
    ...
    fn borrowed() {
        ...
        let got: Foo = deserialize_string_record(&record, Some(&headers)).unwrap();
        assert_eq!(
            got,
            Foo {
                a: "foo",
    ...
    fn borrowed_map() {
        ...
        let got: HashMap<&str, &str> = deserialize_string_record(&record, Some(&headers)).unwrap();

        let expected: HashMap<&str, &str> = headers.iter().zip(&record).collect();
    ...
    fn borrowed_map_bytes() {
        ...
        let got: HashMap<&[u8], &[u8]> = deserialize_byte_record(&record, Some(&headers)).unwrap();

        let expected: HashMap<&[u8], &[u8]> = headers.iter().zip(&record).collect();
        assert_eq!(got, expected);
    }
    ...
    fn partially_invalid_utf8() {
        ...
        let record = ByteRecord::from(vec![b(b"baz"), b(b"foo\xFFbar"), b(b"quux")]);
        let got: Row = deserialize_byte_record(&record, Some(&headers)).unwrap();
        assert_eq!(
            got,
            Row {
                h1: "baz".to_string(),
